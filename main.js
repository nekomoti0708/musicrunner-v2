...
let cachedFolderFiles = null; // fallback入力からキャッシュしたファイル群
...
function initEventListeners() {
    ...
    // フォルダーを開くボタン
    // スマホでは showDirectoryPicker が定義されていても動作が不安定なため、
    // 常に <input webkitdirectory> のフォールバックを使用する
    btnOpenFolder.addEventListener('click', async () => {
        if (window.showDirectoryPicker && !isMobile) {
            try {
                ...
                // その他のエラーの場合のみ input にフォールバック
                if (e.name !== 'AbortError') inputFolder.click();
            }
        } else {
            // スマホ または API 未サポートの場合は input を直接クリック
            inputFolder.click();
        }
    });
